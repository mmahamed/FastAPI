...
async def create_todo(user: user_dependency, db: db_dependency, request: TodoRequest):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail='Authentication Failed')
    new_todo = Todos(**request.model_dump(), owner_id=user.get('id'))
    db.add(new_todo)
    db.commit()
...
async def update_todo(user: user_dependency, db: db_dependency, request: TodoRequest, todo_id: int = Path(gt=0)):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail='Authentication Failed')
    todo = db.query(Todos).filter(Todos.id == todo_id).filter(
